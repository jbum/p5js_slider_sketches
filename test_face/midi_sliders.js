...
// Add map function to replace p5.js map
function map(value, inMin, inMax, outMin, outMax) {
    return (value - inMin) * (outMax - outMin) / (inMax - inMin) + outMin;
}

// Add any other p5.js functions we need
  ...
  for (let part of cookie_parts) {
    ...
    if (name === cookie_name) {
      try {
        ...
        console.log("loading cookie data", data);
        ...
        if (Array.isArray(data)) {
            ...
            if (data.buttonNotes) {
                for (let i = 0; i < data.buttonNotes.length && i < buttons.length; ++i) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let slider_top_y = 24;  // Changed from 60 to 10
